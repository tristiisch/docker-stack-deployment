...
if [ "$INPUT_COPY_STACK_FILE" = "true" ]; then
	...
	DEPLOYMENT_COMMAND_OPTIONS=" --log-level debug --host ssh://$DOCKER_USER_HOST:$INPUT_REMOTE_DOCKER_PORT"
fi
...
if ! [ -z "$INPUT_DOCKER_PRUNE" ] && [ $INPUT_DOCKER_PRUNE = 'true' ] ; then
	...
	yes | docker --log-level debug --host "ssh://$DOCKER_USER_HOST:$INPUT_REMOTE_DOCKER_PORT" system prune -a 2>&1
fi
